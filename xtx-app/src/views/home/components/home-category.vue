  ...
  <div class='home-category' @mouseleave="categoryId=null">
    ...
    <div class="layer">
      ...
      <ul v-if="currCategory && currCategory.brands&&currCategory.brands.length">
        <li class="brand" v-for="sub in currCategory.brands" :key="sub.id">
          <RouterLink to="/">
            <img
              :src="sub.picture"
              alt=""
            />
            <div class="info">
              <p class="place"><i class="iconfont icon-dingwei"></i>{{sub.place}}</p>
              <p class="name ellipsis">{{sub.name}}</p>
              <p class="desc ellipsis-2">{{sub.nameEn}}</p>
            </div>
          </RouterLink>
        </li>
      </ul>
...
export default {
  ...
  setup () {
    ...
    const brand = reactive({
      id: 'brand',
      name: '品牌',
      children: [{ id: 'brandChildren', name: '品牌推荐' }],
      brands: []
    })
    // 计算方法渲染数据
