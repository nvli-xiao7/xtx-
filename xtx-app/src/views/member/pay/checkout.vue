  ...
  <div class="xtx-pay-checkout-page">
    <div class="container">
      ...
      <div class="wrapper">
        ...
        <div class="box-body">
         <CheckoutAddress @DeleteChange="DeleteChange" :list="checkoutInfo.userAddresses" @change="changeAddresses"></CheckoutAddress>
        </div>
        <!-- 商品信息 -->
...
export default {
  ...
  components: { XtxButton, XtxBread, CheckoutAddress },
  setup () {
    // 地址数据
    const checkoutInfo = ref(null)
    const route = useRoute()

    findCheckoutInfo(route.query.orderId).then(data => {
      checkoutInfo.value = data.result
    })

    // 修改地址
    const addressId = ref(null)
    const changeAddresses = (id) => {
      addressId.value = id
    }
    // 删除的地址更新
    const DeleteChange = (res) => {
      if (res.length) {
        checkoutInfo.value.userAddresses = res
      } else {
        checkoutInfo.value.userAddresses = []
      }
    }
    return { checkoutInfo, changeAddresses, DeleteChange }
  }
}
</script>
